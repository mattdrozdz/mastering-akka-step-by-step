...
object SalesOrderProcessor {
  ...
  case class ResolvedDependencies(inputs:Inputs,
                                  ...
                                  books:Map[Int, Book], userMgr:ActorRef,
...
class SalesOrderProcessor  extends FSM[SalesOrderProcessor.State, SalesOrderProcessor.Data] {
  ...
  when(LookingUpEntities, 10 seconds)(transform{
    ...
    case FSM.State(state, ResolvedDependencies(inputs, expectedBooks, Some(u), bookMap, userMgr, bookMgr, creditMgr), _, _, _)
...
//      We do this by piping the result of that Future back to this actor so that it can be handled within the actor itself and not via a callback on the Future.
//    If we handled it via a callback, then we'd have to access the internal state of this FSM actor outside of the actor and its mailbox.
//    That's not something we can do (state-handling functionality is private to the actor), or should try to do, hence piping the result back to yourself for proper handling within the actor.
